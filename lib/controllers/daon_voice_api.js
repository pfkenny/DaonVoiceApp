  ...
  if (customerPin){
    ...
    getCustomerByCallSid(callSid, function(customer){
      if (customer) {
        ...
        {
          respTwiml.say('Please say the last four digits of your registered phone number after the beep.', { voice:'woman', language:'en-gb'});
          respTwiml.record({
            ...
            function (err) {
              if(got_transcription)
              {
                if (transcription_result)
                {
                  ...
                  var listRef2 = myRootRef.child('interactions/');
                  listRef2.child(callSid).set({overall_result: false});
...
exports.transcribe = function(req, res) {
  ...
  console.log("Got transcribe callback with status: " + transcriptionStatus + " and value '" + transciptionText + "' from call: " + callSid);
  getCustomerByCallSid(callSid, function(customer){
    if (customer) {
      //compare year of birth to digits
      //get pin and forwared to register2 to validate pin
      ...
      else if (lastPhoneDigits === transciptionText)
      {
