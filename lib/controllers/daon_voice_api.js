...
function voiceDownloadAsBase64(url, cb) {
  setTimeout(function(){
    var data = "";
    ...
    request.get(url, function (error, response, body) {
      if(error)
      {
        console.log('voiceDownloadAsBase64 got error :' + error.message);
        cb(data);
      }
      else
      { 
        if (response.statusCode === 200) {
         ...
         cb(data);
          ...
          console.log('voiceDownloadAsBase64 got http error :' + response.statusCode);
          cb(data);
        }
      }
    });
...
var authenticateVoice = function(callSid, voiceUrl, callback) {
  console.log('Attempting to retrive recording from url:' + voiceUrl);
  voiceDownloadAsBase64(voiceUrl, function(voiceData){
    if (voiceData && (voiceData.length > 0))
    {
      console.log('Got response from saved recording url');
      console.log('Base64 voice data string is length: ' + voiceData.length);
      getInteractionByCallSid(callSid, function(interaction){
        if(interaction)
        {
          getCustomerByAccountID(interaction.account_number, function(customer){
...
exports.greet = function(req, res) {
  ...
  respTwiml.say('Welcome to the payment verification service.', { voice:'woman', language:'en-gb'});
      ...
      if (customer) {
        if (customer.pin === parseInt(customerPin))
        {
          ...
          respTwiml.record({
            ...
            respTwiml.say('We could not hear your Identity X pass phrase. Please start again.', { voice:'woman', language:'en-gb'});
        } else {
...
exports.verify2 = function(req, res) {
  ...
  if (recordingDuration > 0){
    ...
    authenticateVoice(callSid, voiceUrl, function(authenticationResult)
    {
      ...
      {
        var listRef = myRootRef.child('interactions/');
        listRef.child(callSid).update({overall_result: true});
        getInteractionByCallSid(callSid, function(interaction){
          if(interaction)
          {
            var custAccountNum = interaction.account_number;
            var verificationDate = Date.now();
            console.log('Marking customer ' + custAccountNum +' as verified on: ' + verificationDate);
            var custListRef = myRootRef.child('customers/');
            custListRef.child(custAccountNum).update({last_verified:  verificationDate});
            respTwiml.say('Your verification was successful your payment will be processed. Goodbye.', { voice:'woman', language:'en-gb'});
            respTwiml.hangup();
            res.send(respTwiml);
          } else {
  ...
  getCustomerByCallSid(callSid, function(customer){
    if (customer) {
      ...
      if (transcriptionStatus !== "completed")
      {
        ...
        lr.child(callSid).update({transcription_result: false, transcription_text: transcriptionStatus});
      }
      else if (lastPhoneDigits === transciptionText.replace(/\D/g,''))
      {
        ...
        //persist result
        var listRef = myRootRef.child('interactions/');
        listRef.child(callSid).update({transcription_result: true, transcription_text: transciptionText});
      }
      else
      {
        ...
        //persist result
        var listRef2 = myRootRef.child('interactions/');
        listRef2.child(callSid).update({transcription_result: false, transcription_text: transciptionText});
      }
      res.send(respTwiml);
    } else {
              ...
              enrollVoice(callSid, voiceUrl, function(isEnrolled)
            {
              if (isEnrolled)
              {
                getInteractionByCallSid(callSid, function(interaction){
                  ...
                  {
                    var listRef = myRootRef.child('customers/');
                    listRef.child(interaction.account_number).update({registered: true});
