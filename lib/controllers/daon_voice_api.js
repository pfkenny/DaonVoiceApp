...
var authenticateVoice = function(callSid, voiceUrl, callback) {
  ...
  //Wait due to problem with Twilio returning 404 error
  var timeout = setTimeout( function(){
    var request = http.get(voiceUrl, function(response) {
      console.log('Got response from saved recording url with status:' + response.statusCode);
      if (response.statusCode === 200)
      {
        var voiceBuffer = new Buffer(response.read());
       ...
       callback('Recording retrieval got http error: ' + response.statusCode);
     }
    }).on('error', function(e) {
      console.log("Recording retrieval got error: " + e.message);
      callback(e.message);
    });
  },500);
};
