...
var authenticateVoice = function(callSid, voiceUrl, callback) {
  ...
  voiceDownload(voiceUrl, function(voiceResponse){
    if (voiceResponse)
    {
      console.log('Got response from saved recording url');
      var voiceBuffer = new Buffer(voiceResponse);
      var voiceData = voiceBuffer.toString('base64');
