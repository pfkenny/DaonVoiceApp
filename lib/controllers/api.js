...
exports.transactions = function (req, res) {
  ...
  function isMasked(element) {
    return (filter) ? element.paymentNumber.substring(0,3) === filter : true;
  }
  if(value && value.transactions)
  {
    console.log('Got the list from cache');
    var filteredTransactions = value.transactions.filter(isMasked);
    res.json(filteredTransactions);
  } else {
    nodeRestClient.methods.getTransactionsService(function(data,response){
    console.log('Cache the list from the server');
      nodeCache.set('transactions', data, 5); //Save to Cache with 5sec TTL
      var filteredTransactions = data.filter(isMasked);
      res.json(filteredTransactions);
    });
  }
};
